...
void DMA1_Channel6_IRQHandler(void)
{
	...
	TIM16->DIER |= TIM_DIER_UIE; //прерывание по обновлению
...
void ledInit(void)
{
	...
	RCC->APB2ENR|=RCC_APB2ENR_TIM16EN;//Тактирование таймера 10
	//GPIO setting
	GPIOA->MODER|=GPIO_MODER_MODER6_1;//PA6в режим альтернативной функции
	GPIOA->OSPEEDR|=GPIO_OSPEEDER_OSPEEDR6;//Высокая скорость порта PA6
	GPIOA->AFR[0]|=2<<GPIO_AFRL_AFRL6_Pos;//Альтернативная функция AF2 на PA6
	//Timer setting
  ...
  ;
	...
	DMA1_Channel6->CCR&=~(DMA_CCR_EN);//Выключить ДМА
	DMA1_Channel6->CPAR=(uint32_t)(&TIM3->CCR1);//Адрес регистра сравнения TIM3
	DMA1_Channel6->CMAR=(uint32_t)&ledsBuff;//Адрес буфера
